...
Point jitter(Point p) {
    double offset = 0.000002;
    int latMult = (rand() % 5) - 2;
    int lonMult = (rand() % 5) - 2;
    double lat = p.lat + offset * latMult;
    double lon = p.lon + offset * lonMult;
    return Point(lat, lon);
}

/* Moves a step from start to end and returns the new Node */
Point step(Point start, Point end, int stage) {
    double lat = start.lat + (end.lat - start.lat) / stage;
    double lon = start.lon + (end.lon - start.lon) / stage;
    Point p(lat, lon);
    return jitter(p);
}
...
string html_human_route(const vector<uint64_t> & ids, unordered_map<uint64_t, Node*> & nodes, Point boundary) {
    Route coords;
    Route route;
    Point start, cur, end;
    ...
    coords.push_back(boundary);

    for (int i = 0; i < ids.size(); i++) {
        coords.push_back(Point(nodes[ids[i]]->lat, nodes[ids[i]]->lon));
    }
    ...
    for (int i = 0; i < coords.size()-1; i++) {
        ...
        int n = ceil(d / stepSize);

        for (int j = 0; j < n; j++) {
            cur = step(cur, end, n-j);
            route.push_back(cur);
            totalDistance += Distance(route[route.size()-1], route[route.size()-2]);
        }
    }
...
string html_marker(int id, double lat, double lon) {
    stringstream s;
    s.precision(10);
    s << "\t\t\t\tvar marker_" << id << " = new google.maps.Marker({\n";
...
double Distance(Node one, Node two) {
    return Distance(Point(one.lat, one.lon), Point(two.lat, two.lon));
}
...
int main(int argc, const char *argv[]) {
    ...
    Center home = findHomeLocation(routes, Radii[2]);
    vector<Route> prunedRoutes = pruneAndAddBoundaryPoints(routes, home);
    route = prunedRoutes[0];
    ...
    routeData = toString(prunedRoutes[0]);
    vector<uint64_t> routeIds = osrmExtractRoute(routeData);

    bool movingTowardsP1 = true;
    uint64_t id_one, id_two;
    int ind_one, ind_two;
    Point p1, p2;

    //
    // ONLY ACCOUNTING FOR CASES WHERE ONE END INTERSECTS EPZ
    //
    
    if (Distance(home.p, route[0]) < Distance(home.p, route[route.size()-1])) {
        /* Start of route intersects EPZ, closer to center */
        ...
        /* End of route intersects EPZ, closer to center*/
        
        id_one = routeIds[routeIds.size()-1];
        id_two = routeIds[routeIds.size()-2];
        ind_one = route.size()-1;
        ind_two = route.size()-6;
    }
    
    p1 = Point(nodes[id_one]->lat, nodes[id_one]->lon);
    p2 = Point(nodes[id_two]->lat, nodes[id_two]->lon);
    
    if (Distance(p1, route[ind_one]) < Distance(p1, route[ind_two])) {
        /* route[ind_one] is closer to Node one than route[ind_two], generate route towards Node one */
        movingTowardsP1 = true;
    } else {
        /* route[ind_one] is further from Node one than route[ind_two], generate route towards Node two */
        movingTowardsP1 = false;
    }

    ///////
    // TESTING THE ROUTING ALGO
    ///////

    uint64_t start_node = movingTowardsP1 ? id_one : id_two;
    uint64_t other_node = movingTowardsP1 ? id_two : id_one;
    Point boundary = route[ind_one];
    //double distance = home.r / Radii[0] * 0.125;
    double distance = 0.375;
    Edge* start_edge = nodes[start_node]->findEdge(nodes[other_node]);
    ...
    outfile << html_prefix(home.p.lat, home.p.lon);

    outfile << html_human_route(results[maxWeightIndex], nodes, boundary) << ",\n";
    outfile << html_human_route(results[0], nodes, boundary) << ",\n";
    outfile << html_human_route(results[results.size()-1], nodes, boundary) << ",\n";
    ...
    outfile << html_marker(1, home.p.lat, home.p.lon);
    outfile << html_marker(2, nodes[start_node]->lat, nodes[start_node]->lon);
    outfile << html_marker(3, nodes[other_node]->lat, nodes[other_node]->lon);
